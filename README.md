<div align="center">
  <img width="300" height="300" src="https://lh5.googleusercontent.com/1Q9b2FOLs75_G3quDqYxd8Q9USQFWky4MFQALGVFazYI6uYQJ2Hv8M-VXH-SLKichdJVUPqXBXOQD3v8XRjPI4CnSTIl6KUqW_qPob3Klor9aHA4SU3iei2CAX88J1jYzRC2YhFn">
</div>

Version 1
=============================

Cette version permet de générer des mentions légales classique, sans correction de texte, dans d'autres versions, il y aura un gros travail sur le fond avec plusieurs ajouts.
...
Créer un nouveau dossier à l'endroit de votre choix :
...
Vous pouvez maintenant installer le packet de façon général, afin d'utiliser le binaire sur votre système avec la commande : 

```bash
npm i -g legalwork
```

Une fois l'installation terminé, créer un nouveau dossier, vous pouvez générer les mentions légales de deux façons.

Soit avec des questions depuis le prompt :

```bash
legalwork
```

Soit avec des arguments obligatoire à donner dans le prompt :
...
Une fois les mentions légales généré, vous pouvez voir le fichier HTML dans le répertoir "src" de votre nouveau dossier projet.
...
À l'heure où les sites web, e-boutique et application mobile sont omniprésents dans la vie des consommateurs, les besoins en conformité se font de plus en plus ressentir par les entreprises notamment depuis le RGPD début juin 2018.

Nous avons rencontré RABIER Géraldine, référente de la filière Digital Marketing et Communication de l'école de Ynov Bordeaux, qui est bien ancré dans le milieu de notre projet. Elle nous a expliqué plus en détail les réalités du besoin des mentions légales.
...
Nous avons une date limite à respecter pour la livraison du projet. Ainsi que les tâches à effectuer pendant les semaines d'étude imparties durant l'élective, représentant 2, 3 jours de développement.  

Nous n'autorisons pas de tolérances sur les délais. Le premier livrable sera le cahier des charges, le second sera un oral pour présenter le prototype.  

Nous n'avons pas assez de semaines d'étude sur cette élective pour prétendre la livraisons d'un logiciel complet avec possiblement toutes les évolutions. Nous nous limiterons au minimum à la première version avec le prototype pour la génération des mentions légales incluant la CGU au format HTML.  

Concernant la qualité et le contenu du rendu, il devra être conformes à la charte graphique du projet. Ces normes devront être respectées sur tous les rendus. 
...
En haut à gauche, il nous faut dire à qui la générateur rend service : les utilisateurs.
...
Si le générateur doit être adaptée à un élément extérieur, c'est une fonction de contrainte (FC) et on peut donc relier l'élément extérieur au système.
...
Pour les évolutions, nous avons fait le choix d'intégrer plus tard l'interface REST, car le couplage des composants est moins que contraignant, il est inexistant.  

En effet, si nous développons à l'avenir des services RESTful, ils seront faciles à développer et à déployer : légers, avec un hébergement et une maintenance économiques, ils constituent la solution idéale pour de futur versions.
...
GitFlow est une technique qui se base sur la création de branches. Ce workflow est facile à comprendre et évite beaucoup de conflits. Il permet aussi une meilleure visibilité et gestion des développements.  
...
Il pourra grâce à cela nous remonter les éventuels écarts que nous aurions commis dans la réalisation des fonctionnalités demandé.  

Pour la mise en production, nous pensons la faire sur NPM, et plus tard pour les prochaines versions dans un container docker.  

Celui-ci serait exécuté sur le serveur dédié sur un OS linux de la distribution Ubuntu, avec un DNS par exemple (monprojet.fr), nous pensons utiliser un revers proxy avec en frontal Nginx et en back Apache.  Sinon pourquoi pas l'externaliser chez AWS ou OVH. 
